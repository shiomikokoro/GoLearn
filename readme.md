...
2. `%GOPATH%/bin` 没有可运行程序。
    1. 在1.17版本之后，`go get xxx` 只会获取源码
    2. 输入 `go install xxx` 才会打包为可执行文件
3. 项目缓存
...
6. go天然支持高并发，因为底层代码决定了。
7. go的最大诟病就是异常处理，因为没有异常捕获机制，导致每调一个函数都要接一下函数的error数据，戏称Error是go的一等公民。
